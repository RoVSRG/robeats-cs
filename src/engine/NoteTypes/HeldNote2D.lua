...
function HeldNote2D:new(_game, _track_index, _slot_index, _creation_time_ms, _hit_time_ms, _duration_time_ms)
	...
	function self:cons()
		...
		if _note_obj == nil then
			...
			_note_obj.Body.ZIndex = base_z_index
			_note_obj.Head.Position = UDim2.new(0.5, 0, -1, 0)
			_note_obj.Head.ZIndex = base_z_index + 10
			_note_obj.Tail.Position = UDim2.new(0.5, 0, -1, 0)
			_note_obj.Tail.ZIndex = base_z_index + 1
		end
	...
	function self:update(dt_scale)
		...
		elseif
			...
			or _state == HeldNote2D.State.HoldMissedActive
			or _state == HeldNote2D.State.Passed
		then
			if _state == HeldNote2D.State.Holding then
			...
			if
				_game_audio_manager_get_current_time_ms
