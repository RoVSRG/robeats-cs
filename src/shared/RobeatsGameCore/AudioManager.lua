...
function AudioManager:new(_game)
	...
	-- getters
	function self:get_mode()                return _mode                 end
	function self:get_rate()                return _rate                 end
	function self:get_note_prebuffer_ms()   return _note_prebuffer_time  end
	function self:get_current_time_ms()     return _bgm_time_position*1000 + _audio_time_offset end
	...
	function self:startPlay()
		if _mode ~= AudioManager.Mode.Loading then return end
